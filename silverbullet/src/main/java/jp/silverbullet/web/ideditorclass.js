class IdEditorClass {    
    constructor(div) {
		...
		$("#" + idAdd).on('click', function() {
			$.ajax({
			   type: "GET", 
			   url: me.idPath + "/addNew?type=" + $("#" + idPropType).val(),
		...
		$("#" + idRemove).on('click', function() {
			$.ajax({
			   type: "GET", 
			   url: me.idPath + "/remove?id=" + currentId,
			   success: function(msg){
			   }
			});		
		});
		...
		$("#" + idAddChoice).on('click', function() {
			$.ajax({
			   type: "GET", 
			   url: me.idPath + "/addChoice?id=" + currentId,
			   success: function(msg){
		...
		function updateMainTable() {
			...
			$.ajax({
			   type: "GET", 
			   url: me.idPath + "/properties?type=" + propType,
			   success: function(msg){
			   		headers = msg.header;	
		...
		function updateValue(id, paramName, value) {
			$.ajax({
			   type: "GET", 
			   url: me.idPath + "/update?id=" + id + "&paramName=" + paramName + "&value=" + value,
			   success: function(msg){
			   }
			});			
		}
    	...
    	this.rebuilder = function(application) {
    		me.idPath = "//" + window.location.host + "/rest/" + application + "/id2";
			me.updateTypes();
    	}
	}
